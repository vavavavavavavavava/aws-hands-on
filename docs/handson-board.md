...
4. `アプリケーションに名前を付ける` は任意の分かりやすい名前を入力する（例: `board-web-app`。既定名のままでも可）
5. `サインイン識別子のオプション` は `メールアドレス` にチェックを入れる
6. `自己登録` は `自己登録を有効化` にチェックを入れる
7. `サインアップのための必須属性` は未選択のまま
8. `リターン URL を追加 - オプション` は入力しない
9. 画面右下の `ユーザーディレクトリを作成する` を押す
10. 作成直後に `アプリケーションのリソースを設定する` 画面が表示されたら、この画面は使わず左メニューの `ユーザープール` を押す
11. 一覧から今作成したユーザープールを開く
12. ユーザープール詳細画面で `User pool ID` を控える
13. `アプリケーションをセットアップします` の右側に表示されているアプリケーション名（例: `board-web-app`）を押す
14. 開いたアプリケーション詳細画面で `クライアント ID` を控える
